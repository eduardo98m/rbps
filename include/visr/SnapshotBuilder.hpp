...
namespace visr
{
    namespace detail
    {
        ...
        inline ShapeSnap to_shape_snap(const rbc::Shape &s)
        {
            switch (s.type)
            {
                ...
                //         static_cast<uint32_t>(s.heightmap.cols),
                //         static_cast<uint32_t>(s.heightmap.rows),
                //         s.heightmap.cell_size
                //     };

                // case rbc::ShapeType::Mesh:
                //     return MeshSnap{
                //         static_cast<uint32_t>(s.mesh.vertices.size()),
                //         static_cast<uint32_t>(s.mesh.faces.size())
                //     };

                default:
                    return SpherSnap{ 0.1f }; // unreachable — add assert if needed
            }
        }
                                        ...
                                        double   sim_time)
    {
        ...
        for (uint32_t i = 0; i < jc.count(); ++i)
        {
            ...
            j.anchor_a = bc.position[b1_slot]
                       + m3d::rotate(bc.orientation[b1_slot], jc.r_1[i]);
            j.anchor_b = bc.position[b2_slot]
                       + m3d::rotate(bc.orientation[b2_slot], jc.r_2[i]);

            switch (jc.type[i])
            {
                case rbps::JointType::PRISMATIC: j.type = JointTypeSnap::Prismatic; break;
                case rbps::JointType::REVOLUTE:  j.type = JointTypeSnap::Revolute;  break;
                case rbps::JointType::FIXED:     j.type = JointTypeSnap::Fixed;     break;
            }
