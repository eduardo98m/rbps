#pragma once
#include <raylib.h>
#include <imgui.h>

/**
 * @file CameraSystem.hpp
 * @brief Free-look fly-camera (WASD + Q/E + RMB drag) for the visualizer.
 * @ingroup visr
 *
 * Call `update()` every render frame before `BeginMode3D`. Call
 * `draw_panel()` between `rlImGuiBegin` / `rlImGuiEnd` to expose
 * sensitivity sliders.
 *
 * @par Controls
 * - `W` / `S` — forward / back
 * - `A` / `D` — strafe left / right
 * - `Q` / `LShift` — up
 * - `E` / `LCtrl`  — down
 * - RMB drag — look (yaw + pitch)
 * - Scroll wheel — zoom (FOV)
 * - `Space` — pause / resume (handled in `VisrApp`)
 * - `ESC`   — deselect all   (handled in `SelectionSystem`)
 */

namespace visr
{
    /**
     * @brief Free-look camera with adjustable move speed and look sensitivity.
     * @ingroup visr
     */
    struct CameraSystem
    {
        float move_speed  = 10.0f; ///< Translation speed in world units / second.
        float sensitivity = 0.10f; ///< Look sensitivity, degrees per pixel of mouse delta.

        /** @brief Read input and apply movement / rotation / FOV change to `camera`. */
        void update(Camera3D &camera)
        {
            Vector3 movement = {0, 0, 0};
            Vector3 rotation = {0, 0, 0};

            // ── Translation ───────────────────────────────────────────────
            if (IsKeyDown(KEY_W))                           movement.x += move_speed * GetFrameTime();
            if (IsKeyDown(KEY_S))                           movement.x -= move_speed * GetFrameTime();
            if (IsKeyDown(KEY_A))                           movement.y -= move_speed * GetFrameTime();
            if (IsKeyDown(KEY_D))                           movement.y += move_speed * GetFrameTime();
            if (IsKeyDown(KEY_Q) || IsKeyDown(KEY_LEFT_SHIFT))   movement.z += move_speed * GetFrameTime();
            if (IsKeyDown(KEY_E) || IsKeyDown(KEY_LEFT_CONTROL)) movement.z -= move_speed * GetFrameTime();

            // ── Look (RMB drag) ───────────────────────────────────────────
            if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT))
            {
                rotation.x = GetMouseDelta().x * sensitivity;
                rotation.y = GetMouseDelta().y * sensitivity;
            }

            // ── Zoom (scroll → FOV) ───────────────────────────────────────
            const float scroll = GetMouseWheelMove();
            if (scroll != 0.0f)
            {
                camera.fovy -= scroll * 2.0f;
                if (camera.fovy <   5.0f) camera.fovy =   5.0f;
                if (camera.fovy > 120.0f) camera.fovy = 120.0f;
            }

            UpdateCameraPro(&camera, movement, rotation, 0.0f);
        }

        /** @brief ImGui panel exposing move-speed and sensitivity sliders. */
        void draw_panel()
        {
            ImGui::SetNextWindowSize({280, 130}, ImGuiCond_FirstUseEver);
            ImGui::SetNextWindowPos ({10, 720},  ImGuiCond_FirstUseEver);

            if (!ImGui::Begin("Camera")) { ImGui::End(); return; }

            // Log-scale slider: comfortable range for close contact inspection (0.5)
            // all the way to fast scene navigation (50).
            ImGui::SliderFloat("Move speed",  &move_speed,  0.5f, 50.0f,
                               "%.1f", ImGuiSliderFlags_Logarithmic);
            ImGui::SliderFloat("Look sensitivity", &sensitivity, 0.01f, 0.5f, "%.3f");

            if (ImGui::Button("Reset"))
            {
                move_speed  = 10.0f;
                sensitivity =  0.1f;
            }
            ImGui::SameLine(0, 12);
            ImGui::TextDisabled("(tip: slow down near contacts)");

            ImGui::End();
        }

        /** @brief Build a camera with sensible defaults for an indoor physics scene. */
        static Camera3D make_default()
        {
            Camera3D cam{};
            cam.position   = { 8.0f, 6.0f, 14.0f };
            cam.target     = { 0.0f, 2.0f,  0.0f };
            cam.up         = { 0.0f, 1.0f,  0.0f };
            cam.fovy       = 45.0f;
            cam.projection = CAMERA_PERSPECTIVE;
            return cam;
        }
    };

} // namespace visr