...
{
    /**
     * @brief Maximum corners any single shape's `face_corners` may write.
     *
     * Caller-supplied buffers passed to `face_corners` / `shape_face_corners`
     * are sized to this. Set high enough for the largest convex-hull
     * polygon the engine expects to see (12 = a regular dodecagon cap, with
     * room to spare). Increase if you build hulls with > 16-vertex faces.
     *
     * @ingroup rbc
     */
    constexpr int kMaxFaceCorners = 16;
     ...
     * @param[out] corners Buffer with capacity ≥ 4.
     * @param capacity   Capacity of `corners`; the call writes 0 if < 4.
     * @return Number of corners written (0 or 4).
     *
     * @ingroup rbc
     */
