...
TEST(prismatic_joint_constraint_start_is_zero_for_first)
{
    Fixture f;
    create_prismatic_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});
    ASSERT_EQ(f.jc.constraints[0].ids[0], 0u);
}
...
TEST(prismatic_joint_second_start_is_3)
{
    ...
    ASSERT_EQ(f.jc.constraints[1].ids[0], 3u);
    ASSERT_EQ(f.cc.count(), 6u);
}
...
{
    Fixture f;
    uint32_t id = create_prismatic_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});
...
TEST(remove_joint_shrinks_all_arrays)
{
    ...
    ASSERT_EQ(f.jc.constraints.size(), 1u);
...
TEST(remove_joint_also_removes_child_constraints)
{
    ...
    uint32_t id = create_prismatic_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});
    create_revolute_joint(f.jc, f.cc, {.body_1 = 1, .body_2 = 2});
    ASSERT_EQ(f.cc.count(), 7u);

    remove_joint(f.jc, f.cc, id); // removes prismatic + its 3 constraints

    ASSERT_EQ(f.cc.count(), 4u); // only the revolute's 4 remain
    ASSERT_EQ(f.cc.body_1.size(), 4u);
}

TEST(remove_joint_id_becomes_invalid)
{
    Fixture f;
    // create_fixed_joint should return the IdType (uint32_t) from cc.add()
    uint32_t id = create_fixed_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});

    // Verify it is currently valid
    ASSERT_TRUE(f.jc.contains(id));

    remove_joint(f.jc, f.cc, id);

    // In the new system, 'contains' replaces the old Handle::isValid()
    ASSERT_FALSE(f.jc.contains(id));
}

TEST(remove_joint_surviving_id_stable)
{
    Fixture f;
    uint32_t id_a = create_prismatic_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});
    uint32_t id_b = create_revolute_joint(f.jc, f.cc, {.body_1 = 1, .body_2 = 2});
    uint32_t id_c = create_fixed_joint(f.jc, f.cc, {.body_1 = 2, .body_2 = 3});

    remove_joint(f.jc, f.cc, id_b);

    ASSERT_EQ(f.jc.type[f.jc.index_of(id_a)], JointType::PRISMATIC);
    ASSERT_EQ(f.jc.type[f.jc.index_of(id_c)], JointType::FIXED);
}

TEST(remove_joint_data_intact_after_swap)
{
    Fixture f;
    uint32_t id_a = create_revolute_joint(f.jc, f.cc,
                                          {.body_1 = 10, .body_2 = 11, .aligned_axis = {0, 0, 1}});
    uint32_t id_b = create_fixed_joint(f.jc, f.cc,
                                       {.body_1 = 20, .body_2 = 21});

    remove_joint(f.jc, f.cc, id_a);

    size_t i = f.jc.index_of(id_b);
...
{
    Fixture f;
    uint32_t a = create_prismatic_joint(f.jc, f.cc, {.body_1 = 0, .body_2 = 1});
    uint32_t b = create_revolute_joint(f.jc, f.cc, {.body_1 = 1, .body_2 = 2});
    uint32_t c = create_fixed_joint(f.jc, f.cc, {.body_1 = 2, .body_2 = 3});
    remove_joint(f.jc, f.cc, b);
    remove_joint(f.jc, f.cc, a);
    remove_joint(f.jc, f.cc, c);
    ASSERT_EQ(f.jc.count(), 0u);
    ASSERT_EQ(f.cc.count(), 0u);
...
TEST(mixed_joints_constraint_starts_contiguous)
{
    ...
    ASSERT_EQ(f.jc.count(), 3u);
    ASSERT_EQ(f.cc.count(), 9u);
    ASSERT_EQ(f.jc.constraints[0].ids[0], 0u);
    ASSERT_EQ(f.jc.constraints[1].ids[0], 3u);
    ASSERT_EQ(f.jc.constraints[2].ids[0], 7u);
}
...
TEST_SUITE(
    ...
    RUN_TEST(remove_joint_id_becomes_invalid),
